...
- [x] Unit tests cover separable logic, configuration, serialization, permissions, and failure paths where applicable. **173 tests across 13 classes**: `FmConfigTest` 26, `MarketResolverTest` 34, `LedgerTest` 20, `DiamondsTest` 17, `BukkitConfigSourceTest` 11, `ExperienceMathTest` 11, `MarketCommandTest` 11, `PluginDescriptorTest` 11, `AccountDaoTest` 9, `DatabaseExecutorTest` 8, `EditionResolverTest` 6, `MigrationsTest` 5, `AccountMergeTest` 4. Separable logic is testable at all because four of the five packages are Bukkit-free by constraint. **Failure paths are the majority of the suite**, not an afterthought — overflow refusal, rollback on `Error`, nested-transaction refusal, insufficient funds, malformed and exponential amounts, negative balances refused by the DB `CHECK`, the fail-closed buy-back audit, unknown-outcome handling, and console misuse each have named tests. *Item serialization is not covered because M1 has no items* — `ItemStack` identity and escrow are M2, which is why the check reads "where applicable".
- [x] `PluginDescriptorTest` parses `plugin.yml` and `config.yml` with SnakeYAML and asserts `name`, `main`, a `String`-typed `api-version`, a fully-substituted `version`, every command the code looks up, every permission the code checks, and the declared soft dependencies. All present, plus four assertions beyond the template: that permissions for **unbuilt** milestones are *absent* (a declared-but-unimplemented node tells an operator they have a knob that does nothing), that `farmersmarket.admin`'s `children:` block grants nothing that no longer exists (Paper registers an undeclared child as a permission in its own right), that the `usage:` string names only M1's subcommands, and that the `libraries:` SQLite coordinate carries the same version as `pom.xml` so the two cannot drift.
...
Gate 9 complete `2026-07-22` (v0.1.0), `2026-07-28` (v0.2.0), and `2026-08-04` (v0.3.0). Releases:
...
**`0.3.0` re-verified `2026-08-04` — NO manifest change.** The entry is unchanged and carries **no `pin`**,
so it already follows the latest non-prerelease release, which is now `v0.3.0`; enrollment needed no edit and
no updater-repo commit. Re-verified against a disposable single-entry sandbox (`GITHUB_TOKEN` set to avoid the
unauthenticated rate limit): **fresh** — `would install v0.3.0` (dry) → `installed v0.3.0` (real, 135,570-byte
JAR = the v0.3.0 release asset byte-for-byte, checksum-verified on install); **no-op** — a second real run
reported `already current (v0.3.0)`; **legacy archival** — a planted `farmers-market-0.2.0.jar` was matched by
the glob and moved to `farmers-market-0.2.0.jar.<ts>.legacy.bak`, leaving `farmers-market.jar` (0.3.0) in place;
`json.tool` OK, `unittest` 11/11. Sandboxes discarded; the production `/minecraft` volume was never touched.
